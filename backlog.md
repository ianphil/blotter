...
- [ ] **Final message dropped after many tool calls** `bug` — reducer's `message_final` handler checks `blocks.some(b => b.type === 'text')` — if ANY text block exists from an earlier turn, the final message content is silently dropped. Fix: check `b.sdkMessageId === event.sdkMessageId` instead. Secondary: auto-scroll disengages after many tool blocks expand past 100px threshold. File: `src/renderer/lib/store/reducer.ts` line 88. *(Ian, 2026-04-13)*
